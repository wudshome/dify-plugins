...
class LinkMessageHandler(MessageHandler):
    ...
    def handle(self, message: WechatMessage, session: Any, app_settings: Dict[str, Any]) -> str:
        """处理链接消息"""
        logger.info(f"接收到链接消息，标题: {message.title}, URL: {message.url}")
        ...
        # 构建链接描述文本
        link_text = f"[链接] 标题: {message.title}\n描述: {message.description or '无描述'}\nURL: {message.url}"
        
        # 调用AI接口处理链接信息
        response_data = self._invoke_ai(session, app, link_text, conversation_id)
    ...
    def _invoke_ai(self, session: Any, app: Dict[str, Any], content: str, conversation_id: Optional[str]) -> Dict[str, Any]:
        ...
        self.new_conversation_id = None  # 初始化新会话ID为None
